  ...
  async function submit(values) {
    if (row) { // si estan actualizando
        ...
        action: {
          ...
          onClick: async () => {
            try {
              ...
              await axios.post('/insertBitacora', {Accion: `${user[0][0].NOMBRE} actualizó un control de lavado`});
            } catch (error) {
      ...
      try {
        ...
        closeModal(false);
        setRows(newRows.data);
        await axios.post('/insertBitacora', {Accion: `${user[0][0].NOMBRE} creó un control de lavado`});
      } catch (error) {
        toast.error(error.response.data); // Muestra el mensaje de error
        console.error(error);
      }
